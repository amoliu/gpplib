...
class sa_mdpReplanner2:
    ''' MDP Replanner test class.
    '''
    def __init__(self,riskMapShelf='RiskMap3.shelf',romsDataDirectory='/Users/arvind/data/roms/'):
        self.pngDir = 'pngs_sa_mr_mer_mdp2'
        self.lastGoal = (-1,-1)
        self.numRuns = 10
        try:
        ...
        self.rp = SA_Replanner2(riskMapShelf,romsDataDirectory)
        self.samdp = SA_MDP2(riskMapShelf,romsDataDirectory)
        self.MRrp = Replanner.Replanner(riskMapShelf,romsDataDirectory)
    ...
    def RunMDPandReplanners(self,start,goal,yy,mm,dd,numDays,startT = 1):
        ...
        if goal!= self.lastGoal:
            ...
            self.samdp.GetIndexedPolicy(yy,mm,dd,numDays,goal,theta=thetaVal)
        ...
        plt.text(1.5,10,'Risks: SAmdp=%.2f, RP=%.2f, MR=%.2f'%(sa_totRiskMDP,totRiskRP,totRiskMRP))
        plt.text(1.5,9.5,'Avg. Hops: SAmdp=%.2f, RP=%.2f, MR=%.2f'%(sa_totHopsMDP/float(self.numRuns),totHopsRP/float(self.numRuns),totHopsMRP/float(self.numRuns)))
        plt.text(1.5,9,'Avg.Path-Len: SAmdp=%.2f, RP=%.2f, MR=%.2f'%(sa_totDistMDP/float(self.numRuns),\
                                                    totDistRP/float(self.numRuns), totDistMRP/float(self.numRuns)))
        plt.text(1.5,8.5,'Successes: SAmdp=%d, RP=%d, MR=%d'%(sa_totSuccessMDP,totSuccessRP,totSuccessMRP))
        plt.text(1.5,8,'Non-Starts: SAmdp=%d, RP=%d, MR=%d'%(sa_totNonStartsMDP,totNonStartsRP,totNonStartsMRP))
        plt.text(1.5,7.5,'Crashes: SAmdp=%d, RP=%d, MR=%d'%(sa_totCrashesMDP,totCrashesRP,totCrashesMRP))
        plt.savefig('%s/Ensembles_%04d%02d%02d_%d_%d_%d_%d.png'%(self.pngDir,yy,mm,dd,start[0],start[1],goal[0],goal[1]))
        plt.close()
        
        return saMdpResult, RpResult, MRpResult
    
conf = gpplib.Utils.GppConfig()
mrp = sa_mdpReplanner2(conf.myDataDir+'RiskMap3.shelf',conf.romsDataDir )
yy,mm,dd,numDays = 2012,7,30,1
posNoise, romsNoise = 0.01, 0.01

mrp.samdp.GetTransitionModelFromShelf(yy,mm,dd,numDays,posNoise,romsNoise,conf.myDataDir+'NoisyGliderModels4')
mrp.rp.GetTransitionModelFromShelf(yy,mm,dd,numDays,posNoise,romsNoise,conf.myDataDir+'NoisyGliderModels4')
#mrp.MRrp.GetTransitionModelFromShelf(yy,mm,dd,numDays,posNoise,romsNoise,conf.myDataDir+'NoisyGliderModels4')
mrp.rp.CreateExpRiskGraph()
mrp.MRrp.CreateGraphUsingProximityGraph('MinRisk')
...
nodeLocs = []
for a in mrp.samdp.g.nodes():
    i,j = mrp.samdp.GetXYfromNodeStr(a)
    nodeLocs.append((i,j))

for locA in nodeLocs:
    for locB in nodeLocs:
