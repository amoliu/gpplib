...
class ReportComparison:
    ...
    def GetResultsFromShelves(self,yy,mm,dd,numDays):
        tableResult = shelve.open('rbrp_results/RBRP_Results_%04d%02d%02d.shelve'%(yy,mm,dd),writeback=False)
        '''
        try:
        ...
        try:
            ...
            self.rbrpIndx = self.BuildIndex(self.allRbrpResults)
        except KeyError:
            pass
                ...
                if locA!=locB and not self.gm.GetObs(self.gm.lat_pts[locA[1]],self.gm.lon_pts[locA[0]])\
                    and not self.gm.GetObs(self.gm.lat_pts[locB[1]],self.gm.lon_pts[locB[0]]):
                        ...
                        if self.allRbrpResults!=None:
                            if self.rbrpIndx.has_key('(%d,%d),(%d,%d)'%(start[0],start[1],goal[0],goal[1])):
                                indx = self.rbrpIndx['(%d,%d),(%d,%d)'%(start[0],start[1],goal[0],goal[1])]
