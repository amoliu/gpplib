...
newArray+= 0.2 * np.ones(newArray.shape)
newArray = np.where(newArray>0.9,1,newArray)
...
class PotentialMap():
    ...
    def GetLatLonFromPotMapXY(self,x,y):
        ...
        self.x2lon,self.y2lat = (e-w)/self.w, (n-s)/self.h#rMap.GetLonDiff()/self.w, rMap.GetLatDiff()/self.h
        ...
        return s+(y0+y)*self.pY2lat, w+(x0+x)*self.pX2lon
   
    def GetRmapXYfromLatLon(self,lat,lon):
       x0,y0 = 0,0
       lon0,lat0 = w,s#rMap.GetOxDeg(),rMap.GetOyDeg()
       self.lon2x,self.lat2y = self.w/(e-w),self.h/(n-s)
