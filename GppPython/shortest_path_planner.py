...
class sa_mdpRbReplanner:
	...
	self.yy, self.mm, self.dd, self.numDays = yy,mm,dd,numDays
                    ...
                    if self.MRrp.gs.CollisionReason=='Obstacle':
                        ...
                        if self.MRrp.gs.CollisionReason=='RomsNansAtStart':
                            ...
                            if self.MRrp.gs.CollisionReason == 'RomsNanLater':
                            	...
                            	totNoPolicyMRP +=1
            '''
            tempRiskSpRP, landedSpRP = self.sprp.SimulateAndPlotMR_PolicyExecution(start,goal,startT+i,False,'c-')
        ...
        SpRpResult['MST'] = self.sprp.sp_mst
        ...
        '''
        #plt.text(1.5,10,'Risks: RRP=%.2f'%totRiskSpRP)
        #plt.text(1.5,9.5,'Avg.Hops: RRP=%.2f'%(totHopsSpRP/float(self.numRuns)))
        #plt.text(1.5,9,'Avg.Path-Len: RRP=%.2f'%(totDistSpRP/float(self.numRuns)))
        #plt.text(1.5,8.5,'Successes: RRP=%d'%totSuccessSpRP)
        #plt.text(1.5,8,'Non-Starts: RRP=%d'%totNonStartsSpRP)
        #plt.text(1.5,7.5,'Crashes: RRP=%d'%totCrashesSpRP)
    ...
    for locB in GoodCorrLocsS:
        if locA!=locB  and not mrp.samdp.gm.GetObs(mrp.samdp.gm.lat_pts[locA[1]],mrp.samdp.gm.lon_pts[locA[0]]) \
            ...
            if mrp.samdp.gm.lpGraph.has_node('(%d,%d)'%(locA[0],locA[1])) and mrp.samdp.gm.lpGraph.has_node('(%d,%d)'%(locB[0],locB[1])):
                ...
                '''
                tableResult['GoodCorr_SPRP_%04d%02d%02d'%(yy,mm,dd)] = RbRpResultsG
                tableResult['All_SPRP_%04d%02d%02d'%(yy,mm,dd)] = all_RbRpResults
                tableResult.close()
            
#mdpResultsB,rpResultsB,MRrpResultsB,sMdpResultsB, RbRpResultsB = [], [], [], [], []
#RbRpResultsB, all_RbRpResults = [], []
RbRpResultsB = []
    ...
    for locB in BadCorrLocsS:
        if locA!=locB  and not mrp.samdp.gm.GetObs(mrp.samdp.gm.lat_pts[locA[1]],mrp.samdp.gm.lon_pts[locA[0]]) \
            ...
            if mrp.samdp.gm.lpGraph.has_node('(%d,%d)'%(locA[0],locA[1])) and mrp.samdp.gm.lpGraph.has_node('(%d,%d)'%(locB[0],locB[1])):
                ...
                tableResult['All_SMDP_%04d%02d%02d'%(yy,mm,dd)] = all_sMdpResults
                '''
                tableResult['BadCorr_SPRP_%04d%02d%02d'%(yy,mm,dd)] = RbRpResultsB
                tableResult['All_SPRP_%04d%02d%02d'%(yy,mm,dd)] = all_RbRpResults
                tableResult.close()
