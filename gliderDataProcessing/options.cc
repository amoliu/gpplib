...
//    05/11/01  tc@dinkumsoftware.com
//    - Added include of <cstdlib> after removing exit() declarations
...
/// See if we should be parsing only options or if we also need to parse positional arguments
inline static int
...
/**
 ...
 ^PARAMETERS:
    \param src -- the actual keyword to match
    \param attempt -- the possible keyword to compare against "src"
    \param len -- number of character of "attempt" to consider (if 0 then we should use all of "attempt")
 ...
 ^RETURN-VALUE:
    \return An enumeration value of type kwdmatch_t corresponding to whether
...
// ---------------------------------------------------------------------------
// ^FUNCTION: OptionSpec::Format -
///                       format an option-spec for a usage message
/**
 ^SYNOPSIS:
    unsigned OptionSpec::Format(buf, optctrls) const


    \param buf -- where to print the formatted option
    \param optctrls -- option-parsing configuration flags
...
/*
 ...
 ^PARAMETERS:
    \param opt -- the option-character to match
    \param ignore_case -- should we ignore character-case?
 ...
 ^RETURN-VALUE:
    \return NULL if no match is found, otherwise a pointer to the matching option-spec.
...
// ---------------------------------------------------------------------------
// ^FUNCTION: Options::match_longopt -
/// match a long-option
/**
 ...
 ^PARAMETERS:
    \param  opt -- the long-option to match
    \param len -- the number of character of "opt" to match
    \param ambiguous -- set by this routine before returning.
...
/**
 ...
 ^RETURN-VALUE:
     \return 'c' if the -c option was matched (optarg points to its argument)
     \return BADCHAR if the option is invalid (optarg points to the bad option-character).
