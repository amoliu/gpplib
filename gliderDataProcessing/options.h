...
//
//    10-May-01 tc@DinkumSoftware.com
//        Downloaded  http://www.enteract.com/~bradapp  www.bradapp.net
//        Renamed options.C -> options.h
//    11-May-01 tc@DinkumSoftware.com
...
public:
   ...
   /// returns current item in the iterator
...
public:
   ...
   */
   ...
   operator()(void) = 0;
      ...
      /// resets the "current-element" to the first one in the "list"
   virtual void
   rewind(void) = 0;
} ;

/// Class to iterate through an array of tokens.

/**The array may be terminated by NULL or a count containing
   the number of tokens may be given.
*/
...
public:
   /// constructor, list terminated by NULL
...
public:
   ...
   */
   ...
   /// returns the delimters in use
   const char *
   delimiters(void)  { return  seps; }

   /// sets the delimeters to use
   void
...
/// class for iterating over arguments that come from an input stream
/**
...
/**
 ...
 be NULL. Each non-NULL string in the array must have the following format:
   ...
   The 2nd character must be one of '|', '?', ':', '*', or '+'.
     \li '|' -- indicates that the option takes NO argument;
     \li '?' -- indicates that the option takes an OPTIONAL argument;
     \li ':' -- indicates that the option takes a REQUIRED argument;
     \li '*' -- indicates that the option takes 0 or more arguments;
     \li '+' -- indicates that the option takes 1 or more arguments;
...
\endverbatim
 ...
 option and "-x" is an option that takes no value):
    ...
    \li cmdname -x -l item1 item2 item3 -- arg1 arg2 arg3
    \li cmdname -x -litem1 -litem2 -litem3 -- arg1 arg2 arg3
    \li cmdname -l item1 item2 item3 -x arg1 arg2 arg3
...
public:
   ...
   enum OptCtrl {
      ...
      ANYCASE    = 0x01,
      /// Dont print error messages
      QUIET      = 0x02,
      /// Allow "+" as a long-option prefix
      PLUS       = 0x04,
      /// Dont accept long-options
      SHORT_ONLY = 0x08,
      /// Dont accept short-options(also allows "-" as a long-option prefix).
      LONG_ONLY  = 0x10,

  /** Normally, when we see a short (long) option
                          ...
                          */
      NOGUESSING = 0x20,
    /** By default, Options will not present positional
   ...
   */
   ...
   // tells if options were explicitly terminated
