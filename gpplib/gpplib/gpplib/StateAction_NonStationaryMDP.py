    ...
    def FindNearestNodeOnGraph(self,curNode,t):
        ...
        for a in self.locG.nodes():
            ...
            if not self.gm.ObsDetLatLon(lat1,lon1,lat2,lon2):
                    if self.mdp['U'][t,y,x] > best_utility and dist<self.Dmax and self.mdp['U'][t,y,x]!=0.:
                        best_utility = self.mdp['U'][t,y,x]
                        nearest_node =  (x,y)
        return nearest_node
    ...
    def SimulateAndPlotMDP_PolicyExecution_R(self,simulTime=-1,PostDeltaSimCallback=None,PostSurfaceCallbackFn=None):
        ...
        if self.lastLegComplete == True:
                ...
                try:
                    ...
                    #self.bx,self.by = self.GetPolicyAtCurrentNode((self.a[0],self.a[1]),(self.bx,self.by),tStart)
                    self.bx,self.by = self.GetPolicyAtCurrentNode((self.a[0],self.a[1]),(self.bx,self.by),self.totalTime/3600.)
