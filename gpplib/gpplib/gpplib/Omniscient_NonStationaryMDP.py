...
''' An Omniscient version of the Non-stationary GP-MDP. This planner already has the transition probabilities
for the entire time stretch within which planning will be done.
'''
class O_SA_NS_MDP( object ):
    ...
    def GetPolicyAtCurrentNode(self,curNode,goal,t,forceGoal=False):
        ...
        #t %=48;            
        a = (int(t+0.5), int(curNode[0]+0.5),int(curNode[1]+0.5))
        i,j = None, None
    ...
    def SimulateAndPlotMDP_PolicyExecution_R(self,simulTime=-1,PostDeltaSimCallback=None,PostSurfaceCallbackFn=None):
        ...
        print 'Dist From Goal=%f, AcceptRad=%f'%(self.distFromGoal, self.acceptR)
        ...
        if self.lastLegComplete == True:
                ...
                try:
                    ...
                    #import pdb; pdb.set_trace()
                    self.bx,self.by = self.GetPolicyAtCurrentNode((self.a[0],self.a[1]),(self.bx,self.by),self.totalTime/3600.) # Forcing this for now.
...
if __name__=='__main__':
    ...
    curNoise = None
    start,goal = (0,0), (6,2)
    print 'Omniscient State Action Non-Stationary MDP'
    import gpplib.Utils
