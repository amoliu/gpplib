...
class SA_GP_MDP( object ):
    ...
    def GetTransitionModelFromShelf(self,yy,mm,dd,s_indx,e_indx,posNoise=None,currentNoise=None,shelfDirectory='.'):
        ...
        if posNoise==None and currentNoise==None:
            gmShelf = shelve.open('%s/gliderModelGP2_%04d%02d%02d_%d_%d.shelf'%(shelfDirectory,yy,mm,dd,s_indx,e_indx), writeback=False )
        if posNoise!=None:
            if currentNoise!=None:
                gmShelf = shelve.open('%s/gliderModelGP2_%04d%02d%02d_%d_%d_%.3f_RN_%.3f.shelf'%(shelfDirectory,yy,mm,dd,s_indx,e_indx,posNoise,currentNoise),writeback=False)
            else:
                gmShelf = shelve.open('%s/gliderModelGP2_%04d%02d%02d_%d_%d_%.3f.shelf'%(shelfDirectory,yy,mm,dd,s_indx,e_indx,posNoise), writeback=False)
        if posNoise==None and currentNoise!=None:
            gmShelf=shelve.open('%s/gliderModelGP2_%04d%02d%02d_%d_%d_RN_%.3f.shelf'%(shelfDirectory,yy,mm,dd,s_indx,e_indx,currentNoise), writeback=False)     
            ...
            for b in self.locG.nodes():
                if a != b:
                    ...
                    if self.gm.TransModel.has_key(edge_str):
                            ...
                            self.g.add_edge((i,j),(x,y),weight=rwd,trapped=False)
    ...
    def doValueIteration(self, eps = 0.00001, MaxRuns=50 ):
        ...
        for i in range( 0, MaxRuns ):
            ...
            for nodeNum,node in enumerate(self.g.nodes()):
                ...
                if nodeNum%10==0:
                    print '%d) %.3f'%(nodeNum,time.time()-iterStartTime),(x,y)
                            ...
                            if i!=j:
                                if self.isOnMap(state_prime) and self.isOnMap(state_action):
                                    if not self.gm.ObsDetLatLon(self.gm.lat_pts[yp],self.gm.lon_pts[xp], \
                                        ...
                                        Util += transProbs[j][i] * self.mdp['U'][y+j-zeroLoc,x+i-zeroLoc]
    ...
    def FindNearestNodeOnGraph(self,curNode,t):
        ...
        for a in self.locG.nodes():
            ...
            if not self.gm.ObsDetLatLon(lat1,lon1,lat2,lon2):
                    if self.mdp['U'][y,x] > best_utility and dist<self.Dmax and self.mdp['U'][y,x]!=0.:
    ...
    def SimulateAndPlotMDP_PolicyExecution(self,start,goal,simStartTime,newFig=True,lineType='r-',NoPlot='False'):
        ...
        return self.gs.SimulateAndPlot_PolicyExecution_NS(start,goal,simStartTime,self.maxDepth,self.GetPolicyAtCurrentNode,lineType,newFig)
...
if __name__=='__main__':
    ...
    curNoise = None
    start,goal = (0,0), (6,2)
    print 'State Action Stationary GP MDP'
    import gpplib.Utils
    conf = gpplib.Utils.GppConfig('../../config.shelf')
    saNS_Mdp = SA_NS_MDP(conf.myDataDir+'RiskMap3.shelf',conf.romsDataDir)
    saNS_Mdp.GetTransitionModelFromShelf( yy, mm, dd, 0, 48, posNoise, curNoise, conf.myDataDir+'/gliderModelGP2/' )
