...
class SwitchingMR_Replanner:
    def __init__(self , **kwargs ):
        ''' Keyword Args:
                ...
                plannerType (str) : Type of planner (MRrp/mrplanner)
        '''
        self.conf = gpplib.Utils.GppConfig('../config.shelf')
        riskMapShelf, romsDataDir = \
            self.conf.myDataDir+'RiskMap3.shelf', self.conf.romsDataDir
        gliderModelDir, policyTableDir = \
            self.conf.myDataDir+'NoisyGliderModels4/', 'ReplayPolicies/'
        ...
        self.MRrp = SA_Replanner2( self.riskMapShelf, self.romsDataDir )
        self.MRrp_type_str = 'MRrp'
    ...
    def LoadTransitionModelsFromShelf(self,yy,mm,dd,hr,mi,posNoise=0.01,curNoise=0.01,numDays=0):
        ...
        if self.LastTransModelLoaded != gModName:
            #self.LoadTransModelForPlanner(self.MRrp, gModName)
            #self.MRrp.CreateExpRiskGraph()
            self.MRrp.CreateMinRiskGraph()
    ...
    def GetIndexedPolicy(self,yy,mm,dd,hr,mi,goal,**kwargs):
        ...
        policyStr = 'Pol_MRRP_%04d%02d%02d_%d_%d_G_%d_%d_%.3f_%.3f'%(yy,mm,dd,t1,t2,goal[0],goal[1],posNoise,self.curNoise)
        if self.LastPolicyLoaded != policyStr:
            ...
            if policyTable.has_key(sarp_key):
                ...
                policyForKey={}
                sp_mst, sp_dist = self.MRrp.GetShortestPathMST(goal)
    ...
    def LoadCurrentsFromShelve(self,yy,mm,dd,numDays):
        ...
        if self.LastCurrentDataLoaded!= currentStr:
            ...
            self.MRrp.GetRomsData(yy,mm,dd,numDays)
            self.predGM.GetRomsData(yy,mm,dd,numDays)
            self.MRrp.gm.GetRomsData(yy,mm,dd,numDays)
            self.MRrp.gm.yy,self.MRrp.gm.mm,self.MRrp.gm.dd = yy,mm,dd
    ...
    def PlotAndSaveExecution(self,latArray,lonArray):
        if self.storeAllPngs:
            ...
            self.MRrp.gm.PlotNewRiskMapFig(False)
            td = self.t - self.dtStart;
            self.tHrs = td.total_seconds()/3600.
            self.MRrp.gm.PlotCurrentField(int(self.tHrs)%24)
            tempX, tempY = self.MRrp.gm.GetXYfromLatLon(np.array(latArray), np.array(lonArray))
            ax1.plot(tempX, tempY, self.MRrp.lineType)
            ax1.text(3,5,'Simulation at: %04d-%02d-%02d %.1f hrs'%(self.MRrp.gm.yy,self.MRrp.gm.mm,self.MRrp.gm.dd,self.tHrs))
            ax1.text(3,4,'Goal : (%d,%d)'%(self.goal[0],self.goal[1]))
            # Also show the MDP policy on the other figure.
            ax2=plt.subplot(1,2,2)
            self.MRrp.PlotMRpaths(self.goal,fig1)
            plt.title('Min-Risk MST')
             
            #plt.quiver([self.mdp.a[0]],[self.mdp.a[1]],[self.mdp.bx-int(self.mdp.a[0])],[self.mdp.by-int(self.mdp.a[1])],color='r',lw=2)
            plt.savefig('%s/Ensembles_MRRP_%04d%02d%02d_%05.1f.png'%(self.pngDir,self.MRrp.gm.yy,self.MRrp.gm.mm,self.MRrp.gm.dd,self.tHrs),bbox_inches='tight')
            plt.close()
            
    def RunSwitchingMRRPforNhours(self,yy,mm,dd,hr,mi,start,goal,Nhrs,**kwargs):
        ...
        if kwargs.has_key('lineType'):
            self.MRrp.lineType = kwargs['lineType']
        ...
        if kwargs.has_key('num_hops'):
            self.MRrp.num_hops = kwargs['num_hops']
        else:
            self.MRrp.num_hops = 100
        ...
        self.MRrp.gm.yy,self.MRrp.gm.mm,self.MRrp.gm.dd,self.MRrp.gm.numDays = yy,mm,dd,1
        self.MRrp.InitSARP_Simulation(start,goal,startTime,self.MRrp.lineType,False)
        while( self.t < self.dtEnd ):
            ...
            tempRiskReplanner, landedReplanner = self.MRrp.SimulateAndPlotSARP_PolicyExecution_R(simulTime,self.PlotAndSaveExecution,self.DoPostSurfacingThings)
            print self.MRrp.totalTime
            self.t = self.dtStart + datetime.timedelta( hours= self.MRrp.totalTime/3600. )
            self.totalRisk = self.MRrp.totalRisk;
            self.totalTime = (self.t- self.dtStart).total_seconds()/3600.
            self.totalDistFromGoal = self.MRrp.distFromGoal
            self.totNumHops += 1
            self.totPathLength = self.MRrp.totalPathLength/1000.
            print 'Risk=%f, TotalHops = %d, TotPathLen=%f'%(self.totalRisk,self.totNumHops,self.totPathLength)
            print self.t
            if self.MRrp.possibleCollision == True:
                print 'Possible Collision. Quitting!'
                self.CollisionReason = self.MRrp.CollisionReason
                break
            
            if self.MRrp.isSuccess:
                print 'Reached Goal. Done. Therefore, Quitting!'
                self.isSuccess = self.MRrp.isSuccess
                break
            
            if self.totNumHops >= self.MRrp.num_hops:
