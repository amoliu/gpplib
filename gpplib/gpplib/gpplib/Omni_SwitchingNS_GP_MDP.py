...
class Omni_SwitchingNS_GP_MDP:
    def __init__(self , **kwargs ):
        ...
        self.saNS_Mdp = O_SA_NS_MDP( self.riskMapShelf, self.romsDataDir )
    ...
    def GetIndexedPolicy(self,yy,mm,dd,hr,mi,goal,**kwargs):
        ...
        if self.LastPolicyLoaded != policyStr:
            policyTable = shelve.open('%s/Omni_PolicyTableGP_%04d%02d%02d_%d.shelve'%(shelfDirectory,yy,mm,dd,numDays))
    ...
    def RunSwitchingMDPforNhours(self,yy,mm,dd,hr,mi,start,goal,Nhrs,**kwargs):
        ...
        while( self.t < self.dtEnd ):
            ...
            #self.LoadTransitionModelsFromShelf(yy,mm,dd,0,216,self.posNoise)
            #self.LoadTransitionModelsFromShelf(2013, 8, 12, 0, 216,self.posNoise) # Hard-coding for now.
            self.LoadCurrentsFromShelve(yy, mm, dd, 1 )
            #self.GetIndexedPolicy(yy, mm, dd, hr, mi, goal,curNoise = self.curNoise)
            self.GetIndexedPolicy(2013, 8, 12, hr, mi, goal)
            ...
            print 'Time=',self.saNS_Mdp.totalTime
