...
class ProduceGPTransitionGraph(GliderModel):
    ...
    def __init__(self,riskMapShelfFN='RiskMap3.shelf',sfcst_directory='./',**kwargs ):
        super(ProduceGPTransitionGraph,self).__init__(riskMapShelfFN,sfcst_directory)
    ...
    def CreateTransitionModelFromProxemicGraphAndGPBetweenHours2ForStates(self,yy,mm,dd,s_indx=0,e_indx=48,dmax = 1.5, posNoise = None, sState=0, eState=3983):
        ''' Create the transition models. Meant for a more parallelized version.
            Here, we pass this function a file called "states.txt" which it will process, and then 
            ask for it to handle a partial range.
        '''
        numDays = int((e_indx-s_indx)/24+0.5)
        gm_str,rn_str,pn_str = 'gliderModelGP2_%04d%02d%02d_%d_%d_%d_%d'%(yy,mm,dd,s_indx,e_indx,sState,eState),'',''
        useNoisyStarts = False
        u,v,time1,depth,lat,lon = \
            self.GetGP_RomsData(yy,mm,dd,numDays,True)
        u,v,time1,depth,lat,lon = self.GetRomsData(yy,mm,dd,numDays,True,True)
