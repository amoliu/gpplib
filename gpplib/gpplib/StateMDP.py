''' *StateMDP** - A modified MDP which has a Reward function that depends upon 
...
class MDP(object):
    ...
    def GetRomsData(self,yy,mm,dd,numDays,UpdateSelf=True):
        ...
        u,v,time1,depth,lat,lon = self.gs.gm.GetRomsData(yy,mm,dd,numDays)
        u,v,time1,depth,lat,lon = self.gsR.gm.GetRomsData(yy,mm,dd,numDays)
        if UpdateSelf:
