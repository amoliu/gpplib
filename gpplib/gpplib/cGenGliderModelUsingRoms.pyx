''' This is a Cython version of GenGliderModelUsingRoms which we expect will have speedups over the general version. '''
...
class GliderModel(object):
    ...
    def SimulateDive(self,sLat,sLon,tLat,tLon,maxDepth,u,v,lat,lon,depth,t,FullSimulation=True,HoldValsOffMap=True):
        ...
        xArray,yArray = [],[]
        latArray,lonArray = [],[]
        depthArray = maxDepth/2. *(2-np.cos(2*math.pi/self.Tp * tArray)) - maxDepth/2.
        #import pdb; pdb.set_trace()
        t_prime = float(t)
        totalDist = 0.
        ...
        for i in range(0,len(tArray)):
                    ...
                    if FullSimulation==True:
                        ...
                        '''
                        curU,curV = TriLinearInterpolate(depthArray[i],myLat,myLon,int(tVal),depth,lat,lon,u),\
                                    TriLinearInterpolate(depthArray[i],myLat,myLon,int(tVal),depth,lat,lon,v)
                    else:
                        '''
                         Faster, simulation using depth-averaged bilinear interpolation.
                        '''
                        curU,curV = BilinearInterpolate(myLat,myLon,lat,lon,dAvgU[int(tVal)]), \
                                BilinearInterpolate(myLat,myLon,lat,lon,dAvgV[int(tVal)])
        ...
        return xFin,yFin,latFin,lonFin,latArray,lonArray,depthArray,tArray,possibleCollision,CollisionReason,totalDist
    ...
    def GetRisk(self,lat,lon,OffMapNotObs=True):
    ...
    def GetRomsData(self,yy,mm,dd,numDays,useNewFormat = True):
    ...
    def PlotCurrentField(self,t):
        ...
        q = plt.quiver(aX,aY,U,V)
        p = plt.quiverkey(q,1,height-0.5,0.278,"0.278 m/s",coordinates='data',color='k')
    ...
    def InitSim(self,sLat,sLon,tLat,tLon,maxDepth,startT,FullSimulation=True,HoldValsOffMap=True):
        ...
        self.depthArray = maxDepth/2. *(2-np.cos(2*math.pi/self.Tp * self.tArray)) - maxDepth/2.
        #import pdb; pdb.set_trace()
