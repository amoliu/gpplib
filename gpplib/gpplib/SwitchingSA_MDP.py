...
class SwitchingMdp:
    def __init__(self , **kwargs ):
        ...
        self.saMdp = SA_MDP2( self.riskMapShelf, self.romsDataDir )
        ...
        #self.LoadCurrentsFromShelve(yy, mm, dd, 1 ) # Load currents for atleast 1 day
        self.TransStats = {}
    ...
    def GetBestTimeSlot(self,yy,mm,dd,hr,mi):
        ...
        '''
        timeTables = [ (0,12), (6,18), (12,24), (18,30) ]
        t = time.mktime(datetime.datetime(yy,mm,dd,hr,mi).timetuple())
        
        bm = 0
        '''
        for i in range(0,len(timeTables)-1):
            t1 = time.mktime((datetime.datetime(yy,mm,dd,0,0)+datetime.timedelta(hours=timeTables[i][0])).timetuple())
            t2 = time.mktime((datetime.datetime(yy,mm,dd,0,0)+datetime.timedelta(hours=timeTables[i][1])).timetuple())
            t3 = time.mktime((datetime.datetime(yy,mm,dd,0,0)+datetime.timedelta(hours=timeTables[i+1][0])).timetuple())
            
            if t1<=t and t<=t3:
                if abs(t-t1)<=abs(t3-t):
                    return timeTables[i]
                else:
                    return timeTables[i+1]
                    '''
        i = (hr/6)
        return timeTables[i]
    ...
    def LoadTransitionModelsFromShelf(self,yy,mm,dd,hr,mi,posNoise=0.01,curNoise=0.01,numDays=0):
        ...
        gModName = 'gliderModel%s_%04d%02d%02d_%d_%d_%.3f_RN_%.3f'%(self.mdp_type_str,yy,mm,dd,t1,t2,posNoise,curNoise)
        print gModName
    ...
    def GetIndexedPolicy(self,yy,mm,dd,hr,mi,goal,**kwargs):
        ...
        policyStr = 'Pol_%04d%02d%02d_%d_%d_G_%d_%d_%.3f_%.3f'%(yy,mm,dd,t1,t2,goal[0],goal[1],posNoise,self.curNoise)
        if self.LastPolicyLoaded != policyStr:
            ...
            keyForGoalAndNoise = 'G_%d_%d_PN_%.3f_RN_%.3f_WR_%.3f_WG_%.3f'%(goal[0],goal[1],posNoise,self.curNoise,self.theta['w_r'],self.theta['w_g'])
    ...
    def PlotAndSaveExecution(self,latArray,lonArray):
        if self.storeAllPngs:
            ...
            plt.savefig('%s/Ensembles_%04d%02d%02d_%05.1f.png'%(self.pngDir,self.saMdp.gm.yy,self.saMdp.gm.mm,self.saMdp.gm.dd,self.tHrs),bbox_inches='tight')
            plt.close()
