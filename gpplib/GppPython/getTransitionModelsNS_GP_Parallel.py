...
from gpplib.Gen_NS_GliderModelUsingGPRoms import ProduceNS_GPTransitionGraph
...
def main(argv=None):
    if argv is None:
        argv = sys.argv
    if len(argv)<=5:
        print 'Usage %s s_yy,s_mm,s_dd,e_yy,e_mm,e_dd,sState,eState, [useCurrentTime=1/not-supplied, useCurrentTime=0]'%(argv[0])
        sys.exit(-1)
    s_yy,s_mm,s_dd,e_yy,e_mm,e_dd,sState,eState = int(argv[1]),int(argv[2]),int(argv[3]),int(argv[4]),int(argv[5]),int(argv[6]),int(argv[7]),int(argv[8])
    
    if len(argv)==10:
        useCurrentTime = int(argv[9])
    ...
    ptg = ProduceNS_GPTransitionGraph(conf.riskMapDir+'RiskMap3.shelf',conf.romsDataDir)
    ...
    for (yy,mm,dd) in dr.DateList:
        ...
        transModel = ptg.CreateNS_TransitionModelFromProxemicGraphAndGPBetweenHoursForStates(yy,mm,dd,s_indx,e_indx,1.5,posNoise,sState,eState)
        t_end = time.time()
