...
class ReportComparison:
    def __init__(self,yy=None,mm=None,dd=None,numDays=None,romsDataDirectory=None):
        ...
        self.sprpResultsG, self.sprpResultsB, self.allsprpResults = None, None, None
    ...
    def GetResultsFromShelves(self,yy,mm,dd,numDays):
        tableResult = shelve.open('sprp_results/SPRP_Results_%04d%02d%02d.shelve'%(yy,mm,dd),writeback=False)
        '''
        try:
        ...
        try:
            ...
            self.allsprpResults = tableResult['All_SPRP_%04d%02d%02d'%(yy,mm,dd)]
            print 'Loaded All_SPRP_%04d%02d%02d'%(yy,mm,dd)
            self.sprpIndx = self.BuildIndex(self.allsprpResults)
            self.TotalStats_sprp_All = TotalStatsStore('SPRP','All A.Corr')
            self.TotalStats_sprp_B = TotalStatsStore('SPRP','Bad A.Corr')
            self.TotalStats_sprp_G = TotalStatsStore('SPRP','Good A.Corr')
        except KeyError:
            ...
            for locB in BadCorrLocsS:
                if locA!=locB and not self.gm.GetObs(self.gm.lat_pts[locA[1]],self.gm.lon_pts[locA[0]])\
                    and not self.gm.GetObs(self.gm.lat_pts[locB[1]],self.gm.lon_pts[locB[0]]):
                        ...
                        if self.allsprpResults!=None:
                            if self.sprpIndx.has_key('(%d,%d),(%d,%d)'%(start[0],start[1],goal[0],goal[1])):
                                indx = self.sprpIndx['(%d,%d),(%d,%d)'%(start[0],start[1],goal[0],goal[1])]
                                TotalStats, AvgStats, CrashStats = self.GetStatsForRun(self.allsprpResults[indx])
                                self.TotalStats_sprp_B.AccumulateStats(TotalStats, AvgStats, CrashStats)
                    ...
                    and not self.gm.GetObs(self.gm.lat_pts[locB[1]],self.gm.lon_pts[locB[0]]):
                        ...
                        if self.allsprpResults!=None:
                            if self.sprpIndx.has_key('(%d,%d),(%d,%d)'%(start[0],start[1],goal[0],goal[1])):
                                indx = self.sprpIndx['(%d,%d),(%d,%d)'%(start[0],start[1],goal[0],goal[1])]
                                TotalStats, AvgStats, CrashStats = self.GetStatsForRun(self.allsprpResults[indx])
                                self.TotalStats_sprp_G.AccumulateStats(TotalStats, AvgStats, CrashStats)
                                #self.TotalStats_sprp_B.AccumulateStats(TotalStats, AvgStats, CrashStats)
...
'''
'''if rc.allsprpResults:
    rc.TotalStats_sprp_G.PrintAccumulatedResults(yy,mm,dd)
    rc.TotalStats_sprp_B.PrintAccumulatedResults(yy,mm,dd)
'''
'''
    if rc.mdpResultsB:
