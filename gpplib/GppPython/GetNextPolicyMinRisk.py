...
goto_ll_file_num = 27 # GOTO_L25.MA
...
yy,mm,dd = 2012,7,19 #dt.year, dt.month, dt.day
numDays = 1
...
rp = Replanner(conf.myDataDir+'RiskMap3.shelf',conf.myDataDir+'roms/')
#rp.GetTransitionModelFromShelf(yy,mm,dd,numDays,0.01,0.01,conf.myDataDir+'NoisyGliderModels4')
u,v,time1,depth,lat,lon = rp.gm.GetRomsData(yy,mm,dd,numDays,True,True)
rp.gm.yy,rp.gm.mm,rp.gm.dd,rp.gm.numDays = yy,mm,dd,numDays
rp.GetRomsData(yy,mm,dd,numDays,True,True)
#sarp.CreateExpRiskGraph() # Run a Min-Exp-Risk replanner
rp.CreateGraphUsingProximityGraph('MinRisk')
...
goal_wLat,goal_wLon = 3328.228, -11819.370
goalX,goalY = rp.gm.GetXYfromLatLon(goalLat,goalLon)
...
start_wLat,start_wLon = 3332.392, -11837.156
...
startX, startY = rp.gm.GetXYfromLatLon(startLat,startLon)
...
for loc in path_to_goal:
    x,y = rp.gm.GetXYfromNodeStr(loc)
    lat,lon = rp.gm.GetLatLonfromXY(x,y)
...
nHrsHence = 0.0
plt.figure(); rp.gm.PlotNewRiskMapFig(); rp.gm.PlotCurrentField(rtc.GetRomsIndexNhoursFromNow(yy,mm,dd, nHrsHence))
totRiskMER = 0
numFailsMER = 0
