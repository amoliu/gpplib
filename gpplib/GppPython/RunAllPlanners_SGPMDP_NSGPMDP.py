...
from gpplib.SwitchingNS_GP_MDP import *
from gpplib.Omni_SwitchingNS_GP_MDP import *
...
rNsGpMdp = SwitchingNS_GP_MDP(StoreAllPngs = False); rNsGpMdpPr = PlannerResult(PlannerType='Non-Stationary GPMDP')

#rNsGpMdp.LoadTransitionModelsFromShelf(yy,mm,dd,0,48)
...
rOmniGp = Omni_SwitchingNS_GP_MDP( StoreAllPngs = False, theta=thetaVal); rOmniGpPr = PlannerResult(PlannerType='OmniGP_MDP')

#rOmniGpMdp.LoadTransitionModelsFromShelf(2013,8,12,0,216)

#rmdpr = SwitchingMdp(StoreAllPngs=False, theta=thetaVal); rmdprPr = PlannerResult(PlannerType='SwitchingSA_MDP')
#rmrrp.saMdp.gm.PlotNewRiskMapFig(False)
...
startX,startY = rNsGpMdp.saNS_Mdp.gm.GetXYfromLatLon(start_Lat,start_Lon)
...
goalX,goalY = rNsGpMdp.saNS_Mdp.gm.GetXYfromLatLon(goal_Lat,goal_Lon)

simul_end = datetime.datetime(2013,8,23,23,0)
...
wayPtList2 = [(33.543039,-118.331636),(33.523829,-118.588117),(33.488213,-118.701459),(33.522344,-118.757254)]
    
for wptStart in wayPtList:
    for wptGoal in wayPtList:
        startX,startY = rNsGpMdp.saNS_Mdp.gm.GetXYfromLatLon(wptStart[0],wptStart[1])
        goalX,goalY = rNsGpMdp.saNS_Mdp.gm.GetXYfromLatLon(wptGoal[0],wptGoal[1])
...
=======
            for i in range(0,35):
                newdt =dt +  datetime.timedelta(hours=2*i)
>>>>>>> branch 'master' of git@git.robotics.usc.edu:resl-aquatic/gpplib.git
                ...
                '''
                rsarp.RunSwitchingSARPforNhours(yy,mm,dd,hr,mi,start,goal,200,posNoise=posNoise,curNoise=curNoise1,lineType='y.-',simul_end=simul_end,num_hops=250)
                ...
                rmdprGp.RunSwitchingMDPforNhours(yy,mm,dd,hr,mi,start,goal,200,posNoise=posNoise,curNoise=curNoise1,lineType='m.-')
                ...
                rmdpr.RunSwitchingMDPforNhours(yy,mm,dd,hr,mi,start,goal,200,posNoise=posNoise,curNoise=curNoise1,lineType='r.-',simul_end=simul_end,num_hops=250)
                ...
                rNsGpMdp.RunSwitchingMDPforNhours(yy,mm,dd,hr,mi,start,goal,200,lineType='r.-',simul_end=simul_end,num_hops=250)
                rNsGpMdpPr.UpdateWithResultOfRun(DescStr='SA_NS_GPMDP_%04d%02d%02d_%02d%02d'%(yy,mm,dd,hr,mi), \
                            start=start,goal=goal,Risk=rNsGpMdp.totalRisk,Time=rNsGpMdp.totalTime,numHops=rNsGpMdp.totNumHops,
                            pathLen=rNsGpMdp.totPathLength,CollisionReason=rNsGpMdp.CollisionReason,distFromGoal=rNsGpMdp.totalDistFromGoal,
                            isSuccess=rNsGpMdp.isSuccess)
                
                rOmniGp.RunSwitchingMDPforNhours(yy,mm,dd,hr,mi,start,goal,200,lineType='r.-',simul_end=simul_end,num_hops=250)
                rOmniGpPr.UpdateWithResultOfRun(DescStr='Omni_NS_GPMDP_%04d%02d%02d_%02d%02d'%(yy,mm,dd,hr,mi), \
                            start=start,goal=goal,Risk=rOmniGp.totalRisk,Time=rOmniGp.totalTime,numHops=rOmniGp.totNumHops,
                            pathLen=rOmniGp.totPathLength,CollisionReason=rOmniGp.CollisionReason,distFromGoal=rOmniGp.totalDistFromGoal,
                            isSuccess=rOmniGp.isSuccess)
...
totSwNSGP, avgSwNSGP = rNsGpMdpPr.ComputeTotalsAndAverages()
totOmniNSGP, avgOmniNSGP= rOmniGpPr.ComputeTotalsAndAverages()
#totSwSARP,avgSwSARP = rsarpPr.ComputeTotalsAndAverages()

# Save results...
plannerResults = shelve.open('GPMDP_NSGPMDP_Omni_RMRRP_240.shelve')
plannerResults['rmdprGpPr'] = rmdprGpPr.Result
plannerResults['rNsGpMdpPr'] = rNsGpMdpPr.Result
plannerResults['rmrrpPr'] = rmrrpPr.Result
plannerResults['omniGpPr'] = rOmniGpPr.Result

NSGPwith, NSGPperp, NSGPagainst = ComputeAverageVsCurrents( rNsGpMdpPr ); SaveToCsv( rNsGpMdpPr, 'NS_GP_MDP_Runs.csv' )
GPMDPwith, GPMDPperp, GPMDPagainst = ComputeAverageVsCurrents( rmdprGpPr ); SaveToCsv( rmdprGpPr, 'GP_MDP_Runs.csv' )
MRRPwith, MRRPperp, MRRPagainst = ComputeAverageVsCurrents( rmrrpPr ); SaveToCsv( rmrrpPr, 'MrrpRuns.csv' )
OmniMDPwith, OmniMDPperp, OmniMDPagainst = ComputeAverageVsCurrents(rOmniGpPr ); SaveToCsv( rOmniGpPr, 'OmniRuns.csv')
