...
class sa_mdpRbReplanner:
	...
	self.myDataDir = myDataDir
        ...
        '''
        self.sprp = Replanner(riskMapShelf,romsDataDirectory,myDataDir)
    ...
    def RunMDPandReplanners(self,start,goal,yy,mm,dd,numDays,startT = 1):
        saMdpResult, RpResult, MRpResult, sMdpResult, SpRpResult = {}, {}, {}, {}, {}
	self.yy, self.mm, self.dd, self.numDays = yy,mm,dd,numDays
        if goal!= self.lastGoal:
            ...
            plt.close()
            '''
            self.sp_mstSP, self.distSP = self.sprp.GetShortestPathMST(goal)
            self.lastGoal = goal
        ...
        sMdpResult['finalLoc'] = []
        '''
        SpRpResult['Risk'], SpRpResult['pathLength'], SpRpResult['Time'] = [], [], []
        SpRpResult['numHops'], SpRpResult['distFromGoal'], SpRpResult['CollisionReason'] = [], [], []
        SpRpResult['isSuccess']= []; SpRpResult['startTime'] = []
        SpRpResult['finalLoc'] = []
        ...
        '''
        totRiskSpRP, totHopsSpRP, totSuccessSpRP, totCrashesSpRP, totFalseStartsSpRP, totTimeSpRP, totDistSpRP = 0.,0.,0.,0.,0.,0.,0.
        totNoPolicySpRP, totNonStartsSpRP, totNanSpRP = 0,0,0
                    ...
                    if self.MRrp.gs.CollisionReason=='Obstacle':
                        ...
                        if self.MRrp.gs.CollisionReason=='RomsNansAtStart':
                            ...
                            if self.MRrp.gs.CollisionReason == 'RomsNanLater':
                            	...
                            	totNoPolicyMRP +=1
            '''
            tempRiskSpRP, landedSpRP = self.sprp.SimulateAndPlotMR_PolicyExecution(start,goal,self.sp_mstSP,startT+i,False,'c-')
            SpRpResult['Risk'].append(self.sprp.gs.totalRisk); SpRpResult['pathLength'].append(self.sprp.gs.totalPathLength/1000.)
            SpRpResult['Time'].append(self.sprp.gs.totalTime/3600.); SpRpResult['numHops'].append(self.sprp.gs.numHops)
            SpRpResult['CollisionReason'].append(self.sprp.gs.CollisionReason); SpRpResult['isSuccess'].append(self.sprp.gs.isSuccess)
            SpRpResult['distFromGoal'].append(self.sprp.gs.distFromGoal); SpRpResult['startTime'].append(startT+i)
            SpRpResult['finalLoc'].append((self.sprp.gs.finX,self.sprp.gs.finY))
            totRiskSpRP+=self.sprp.gs.totalRisk; totHopsSpRP+=self.sprp.gs.numHops; totTimeSpRP+=self.sprp.gs.totalTime/3600.; totDistSpRP+=self.sprp.gs.totalPathLength/1000.
            if self.sprp.gs.isSuccess==True:
                    totSuccessSpRP+=1
                
            if self.sprp.gs.CollisionReason!=None:
                    if self.sprp.gs.CollisionReason=='Obstacle':
                        totCrashesSpRP+=1
                    else:
                        if self.sprp.gs.CollisionReason=='RomsNansAtStart':
                            totNonStartsSpRP+=1
                        else:
                            if self.sprp.gs.CollisionReason == 'RomsNanLater':
                                totNanSpRP +=1
                            elif self.sprp.gs.CollisionReason == 'DidNotFindPolicy':
                                totNoPolicySpRP +=1
        ...
        '''
        SpRpResult['Successes'] = totSuccessSpRP; SpRpResult['numRuns']= self.numRuns;
        SpRpResult['RomsNans'] = totNanSpRP
        SpRpResult['start'] = start; SpRpResult['goal'] = goal
        SpRpResult['MST'] = self.sp_mstSP
        #SpRpResult['U'] = self.sprp.samdp.mdp['U']
        SpRpResult['TotalRisk'] = totRiskSpRP; SpRpResult['Crashes'] = totCrashesSpRP; SpRpResult['NonStarts'] = totNonStartsSpRP;
        ...
        '''
        plt.text(1.5,10,'Risks: SpRP=%.2f, MDP=%.2f, SAmdp=%.2f, RP=%.2f, MR=%.2f'%(totRiskSpRP,s_totRiskMDP,sa_totRiskMDP,totRiskRP,totRiskMRP))
        plt.text(1.5,9.5,'Avg. Hops: SpRP=%.2f, MDP=%.2f, SAmdp=%.2f, RP=%.2f, MR=%.2f'%(totHopsSpRP/float(self.numRuns),s_totHopsMDP/float(self.numRuns),sa_totHopsMDP/float(self.numRuns),totHopsRP/float(self.numRuns),totHopsMRP/float(self.numRuns)))
        plt.text(1.5,9,'Avg.Path-Len: SpRP=%.2f, MDP=%.2f, SAmdp=%.2f, RP=%.2f, MR=%.2f'%(totDistSpRP/float(self.numRuns),s_totDistMDP/float(self.numRuns),sa_totDistMDP/float(self.numRuns),\
                                                    totDistRP/float(self.numRuns), totDistMRP/float(self.numRuns)))
        plt.text(1.5,8.5,'Successes: SpRP=%d, SMDP=%d, SAmdp=%d, RP=%d, MR=%d'%(totSuccessSpRP,s_totSuccessMDP,sa_totSuccessMDP,totSuccessRP,totSuccessMRP))
        plt.text(1.5,8,'Non-Starts: SpRP=%d, SMDP=%d, SAmdp=%d, RP=%d, MR=%d'%(totNonStartsSpRP,s_totNonStartsMDP,sa_totNonStartsMDP,totNonStartsRP,totNonStartsMRP))
        plt.text(1.5,7.5,'Crashes: SpRP=%d, SMDP=%d, SAmdp=%d, RP=%d, MR=%d'%(totCrashesSpRP,s_totCrashesMDP,sa_totCrashesMDP,totCrashesRP,totCrashesMRP))
        '''
        plt.text(1.5,10,'Risks: SpRP=%.2f'%totRiskSpRP)
        plt.text(1.5,9.5,'Avg.Hops: SpRP=%.2f'%(totHopsSpRP/float(self.numRuns)))
        plt.text(1.5,9,'Avg.Path-Len: SpRP=%.2f'%(totDistSpRP/float(self.numRuns)))
        plt.text(1.5,8.5,'Successes: SpRP=%d'%totSuccessSpRP)
        plt.text(1.5,8,'Non-Starts: SpRP=%d'%totNonStartsSpRP)
        plt.text(1.5,7.5,'Crashes: SpRP=%d'%totCrashesSpRP)
        plt.savefig('%s/Ensembles_%04d%02d%02d_%d_%d_%d_%d.png'%(self.pngDir,self.yy,self.mm,self.dd,start[0],start[1],goal[0],goal[1]))
        plt.close()
        
        #return saMdpResult, RpResult, MRpResult, sMdpResult, SpRpResult
        return SpRpResult
...
'''
mrp.sprp.GetTransitionModelFromShelf(yy,mm,dd,numDays,posNoise,romsNoise,conf.myDataDir+'NoisyGliderModels2')
mrp.sprp.CreateGraphUsingProximityGraph('ShortestPath')
'''
...
mrp.sprp.GetRomsData(yy,mm,dd,numDays)
...
#mdpResultsG,rpResultsG,MRrpResultsG, sMdpResultsG, SpRpResultsG = [], [], [], [], []
#allMdpResults,allRpResults,allMRrpResults, all_sMdpResults, all_SpRpResults = [], [], [], [], []

SpRpResultsG, all_SpRpResults = [], []
    ...
    for locB in GoodCorrLocsS:
        if locA!=locB  and not mrp.samdp.gm.GetObs(mrp.samdp.gm.lat_pts[locA[1]],mrp.samdp.gm.lon_pts[locA[0]]) \
            ...
            if mrp.samdp.gm.lpGraph.has_node('(%d,%d)'%(locA[0],locA[1])) and mrp.samdp.gm.lpGraph.has_node('(%d,%d)'%(locB[0],locB[1])):
                ...
                #mdp_res, rp_res, MRrp_res, sMdp_res, sprp_res = mrp.RunMDPandReplanners(start, goal, yy, mm, dd, numDays, startT)
                sprp_res = mrp.RunMDPandReplanners(start,goal,yy,mm,dd,numDays,startT)
                #mdpResultsG.append(mdp_res); rpResultsG.append(rp_res); MRrpResultsG.append(MRrp_res); sMdpResultsG.append(sMdp_res); SpRpResultsG.append(sprp_res)
                SpRpResultsG.append(sprp_res)
                #allMdpResults.append(mdp_res); allRpResults.append(rp_res); allMRrpResults.append(MRrp_res); all_sMdpResults.append(sMdp_res); all_SpRpResults.append(sprp_res)
                all_SpRpResults.append(sprp_res)
                #tableResult = shelve.open('SA_MDP_MER_Results_%04d%02d%02d.shelve'%(yy,mm,dd))
                tableResult = shelve.open('SPRP_Results_%04d%02d%02d.shelve'%(yy,mm,dd))
                '''
                tableResult['GoodCorr_MDP_%04d%02d%02d'%(yy,mm,dd)] = mdpResultsG
                tableResult['GoodCorr_RP_%04d%02d%02d'%(yy,mm,dd)] = rpResultsG
                tableResult['GoodCorr_MSpRP_%04d%02d%02d'%(yy,mm,dd)] = MRrpResultsG
                ...
                tableResult['All_MSpRP_%04d%02d%02d'%(yy,mm,dd)] = allMRrpResults
                tableResult['All_SMDP_%04d%02d%02d'%(yy,mm,dd)] = all_sMdpResults
                '''
                tableResult['GoodCorr_SPRP_%04d%02d%02d'%(yy,mm,dd)] = SpRpResultsG
                tableResult['All_SPRP_%04d%02d%02d'%(yy,mm,dd)] = all_SpRpResults
                tableResult.close()
            
#mdpResultsB,rpResultsB,MRrpResultsB,sMdpResultsB, SpRpResultsB = [], [], [], [], []
SpRpResultsB, all_SpRpResults = [], []
    ...
    for locB in BadCorrLocsS:
        if locA!=locB  and not mrp.samdp.gm.GetObs(mrp.samdp.gm.lat_pts[locA[1]],mrp.samdp.gm.lon_pts[locA[0]]) \
            ...
            if mrp.samdp.gm.lpGraph.has_node('(%d,%d)'%(locA[0],locA[1])) and mrp.samdp.gm.lpGraph.has_node('(%d,%d)'%(locB[0],locB[1])):
                ...
                #mdp_res, rp_res, MRrp_res, sMdp_res, sprp_res = mrp.RunMDPandReplanners(start, goal, yy, mm, dd, numDays, startT)
                sprp_res = mrp.RunMDPandReplanners(start, goal, yy, mm, dd, numDays, startT)
                #mdpResultsB.append(mdp_res); rpResultsB.append(rp_res); MRrpResultsB.append(MRrp_res); sMdpResultsB.append(sMdp_res); SpRpResultsB.append(sprp_res)
                SpRpResultsB.append(sprp_res);
                #allMdpResults.append(mdp_res); allRpResults.append(rp_res); allMRrpResults.append(MRrp_res); all_sMdpResults.append(sMdp_res); all_SpRpResults.append(sprp_res)
                all_SpRpResults.append(sprp_res)
                tableResult = shelve.open('SPRP_Results_%04d%02d%02d.shelve'%(yy,mm,dd))
                '''
                ...
                tableResult['BadCorr_MSpRP_%04d%02d%02d'%(yy,mm,dd)] = MRrpResultsB
                ...
                tableResult['All_SMDP_%04d%02d%02d'%(yy,mm,dd)] = all_sMdpResults
                '''
                tableResult['GoodCorr_SPRP_%04d%02d%02d'%(yy,mm,dd)] = SpRpResultsB
                tableResult['All_SPRP_%04d%02d%02d'%(yy,mm,dd)] = all_SpRpResults
                tableResult.close()
