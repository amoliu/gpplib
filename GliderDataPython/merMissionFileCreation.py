...
GOTO_L_FILE_NUM = 96
N = 8 # No-Comms timeout in hours
M = 5 # Surface every timeout in hours
MI_FILENAME = 'MISSTRTC.MI'
# Now for the tricky one: UTC time
# Let us assume that we want to pick up the glider at 10:00 am PST on July 18, 2012
yy,mm,dd,hr,mi = 2013,8,8,8,0
...
surf_every_M_hrs= SurfaceGliderBehavior(start_when=gldrEnums.start_when_enums['BAW_EVERY_SECS'], \
                    when_secs=(M*60*60) )
...
gldr_beh_list.Append(surf_if_no_comms_for_N_hrs)
gldr_beh_list.Append(surf_at_every_wpt)
#gldr_beh_list.Append(surf_every_M_hrs)
...
''' Goto-List behavior '''
goto_ll_behavior = GotoListGliderBehavior(args_from_file=GOTO_L_FILE_NUM)
...
gldr_beh_list.SaveMissionFile( MI_FILENAME )
